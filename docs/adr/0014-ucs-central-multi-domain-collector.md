...
The cost, stated plainly: one login plus 13 queries per collected domain
(pinned by `test_scales_query_count_independently_of_fleet_size` —
`equipmentPsu` and `graphicsCard` both added 2026-09-02, see
docs/cisco-collectors.md's "Power supplies (PSUs)" and "GPUs
(coprocessor cards vs. graphics cards)") instead of 9 queries in total,
and a UCS Manager account that works on every domain. For the ~8-domain fleet this was built for that is a minute or
