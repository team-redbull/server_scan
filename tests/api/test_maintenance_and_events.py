...
# --- Classification rule CRUD audit trail ---


async def test_create_classification_rule_records_created_event(
    app_context: tuple[AsyncClient, MongoServerRepository, MongoHealthPolicyRepository],
) -> None:
    client, _repo, _policy_repo = app_context
    ...
    events = (
        await client.get("/api/v1/events", params={"event_type": "CLASSIFICATION_RULE_CREATED"})
    ).json()
    matching = [e for e in events["items"] if e["data"].get("rule_id") == rule_id]
    assert len(matching) == 1


async def test_update_classification_rule_records_updated_event(
    app_context: tuple[AsyncClient, MongoServerRepository, MongoHealthPolicyRepository],
) -> None:
    client, _repo, _policy_repo = app_context
    ...
    await client.patch(f"/api/v1/classification-rules/{rule_id}", json={"description": "updated"})

    events = (
        await client.get("/api/v1/events", params={"event_type": "CLASSIFICATION_RULE_UPDATED"})
    ).json()
    matching = [e for e in events["items"] if e["data"].get("rule_id") == rule_id]
    assert len(matching) == 1


async def test_delete_classification_rule_records_deleted_event(
    app_context: tuple[AsyncClient, MongoServerRepository, MongoHealthPolicyRepository],
) -> None:
    client, _repo, _policy_repo = app_context
    create_resp = await client.post(
    ...
    resp = await client.delete(f"/api/v1/classification-rules/{rule_id}")
    assert resp.status_code == 204

    events = (
        await client.get("/api/v1/events", params={"event_type": "CLASSIFICATION_RULE_DELETED"})
    ).json()
    matching = [e for e in events["items"] if e["data"].get("rule_id") == rule_id]
    assert len(matching) == 1


# --- Health policy CRUD audit trail ---


async def test_create_health_policy_records_created_event(
    app_context: tuple[AsyncClient, MongoServerRepository, MongoHealthPolicyRepository],
) -> None:
    client, _repo, _policy_repo = app_context
    resp = await client.post(
        ...
        json={
            ...
            "category": "storage",
            "severity": "CRITICAL",
            "condition": {"metric": "storage.failed_drive_count", "operator": "GTE", "value": 1},
    ...
    events = (
        await client.get("/api/v1/events", params={"event_type": "HEALTH_POLICY_CREATED"})
    ).json()
    matching = [e for e in events["items"] if e["data"].get("policy_id") == policy_id]
    assert len(matching) == 1


async def test_disabling_health_policy_records_disabled_not_updated_event(
    app_context: tuple[AsyncClient, MongoServerRepository, MongoHealthPolicyRepository],
) -> None:
    client, _repo, _policy_repo = app_context
    create_resp = await client.post(
    ...
    await client.patch(f"/api/v1/health-policies/{policy_id}", json={"enabled": False})

    events = (
        await client.get("/api/v1/events", params={"event_type": "HEALTH_POLICY_DISABLED"})
    ).json()
    matching = [e for e in events["items"] if e["data"].get("policy_id") == policy_id]
    assert len(matching) == 1


async def test_delete_health_policy_records_deleted_event(
    app_context: tuple[AsyncClient, MongoServerRepository, MongoHealthPolicyRepository],
) -> None:
    client, _repo, _policy_repo = app_context
    create_resp = await client.post(
        "/api/v1/health-policies",
        json={
            "name": "audit-really-delete-policy",
            "category": "storage",
            "severity": "CRITICAL",
            "condition": {"metric": "storage.failed_drive_count", "operator": "GTE", "value": 1},
            "message_template": "failed",
    ...
    resp = await client.delete(f"/api/v1/health-policies/{policy_id}")
    assert resp.status_code == 204

    events = (
        await client.get("/api/v1/events", params={"event_type": "HEALTH_POLICY_DELETED"})
    ).json()
    matching = [e for e in events["items"] if e["data"].get("policy_id") == policy_id]
    assert len(matching) == 1
